...
pub fn transcode(read: &mut impl io::Read, write: &mut impl io::Write, encoding: &'static enc::Encoding, input_buffer: &mut [u8], output_buffer: &mut [u8], opt: option::Opt) 
...
mod tests {
    ...
    macro_rules! test_transcode {
        ($name:ident, $input_file:expr, $expected_file:expr, $enc:expr) => {
            ...
            fn $name() {
                ...
                super::transcode(ifile_handle, output, enc.unwrap(), input_buffer, output_buffer, opt).unwrap();
                let test_data = path::Path::new("test_data");
    ...
    // test_transcode!(transcode_sjis_utf82        , "sjis_ja2.txt"         , "utf8_ja2.txt"     , "utf8");
}
